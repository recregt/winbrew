...
#[test]
fn portable_engine_installs_raw_payload_and_removes_directory() -> Result<()> {
    let temp_root = tempdir()?;
    let download_path = temp_root.path().join("tool.exe");
    let install_dir = temp_root.path().join("packages").join("Contoso.Portable");

    fs::write(&download_path, b"portable-binary")?;

    let installer = catalog_installer(InstallerType::Portable, "tool.exe");
    let resolved_kind = probe_installer_from_download(&installer, &download_path)?;
    assert_eq!(resolved_kind, InstallerType::Portable);
    ...
    assert_eq!(receipt.engine_kind, EngineKind::Portable);
    assert_eq!(receipt.install_dir, install_dir.to_string_lossy());
    assert!(install_dir.join("tool.exe").exists());
    assert_eq!(fs::read(install_dir.join("tool.exe"))?, b"portable-binary");
