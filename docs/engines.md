...
| `InstallerType::Zip` | `EngineKind::Zip` | Supported | WinBrew-owned filesystem engine | ZIP is the archive front door today. The shared archive dispatcher (`crates/engines/src/archive/install.rs`) extracts ZIP, Tar, GZip, and 7z payloads once a runtime is resolved; the `SearchPathW` check for a global `7z.exe` lives in `crates/windows`, and the bootstrap-a-local-copy-after-confirmation flow lives in the install flow (`crates/app/src/operations/install/sevenz.rs`), not in the archive dispatcher itself. Remove is still plain directory cleanup. The bootstrapped runtime now writes `bin/7zip/VERSION`, which Renovate can track through the 7-Zip bootstrap version constant. The install flow now probes the downloaded payload before final engine selection, so a mislabeled MSI or MSIX-shaped ZIP can still be routed correctly. |
| `InstallerType::Portable` | `EngineKind::Portable` | Supported | WinBrew-owned filesystem engine | Copies raw payloads into a staging tree, then replaces the target install directory. Raw-only fallback when the downloaded payload cannot be identified structurally; archive-shaped, MSI-like, or MSIX-like payloads can override a misleading manifest before the engine is committed. Remove is plain directory cleanup. |
...
- 7z runtime ownership lives in the install flow: WinBrew checks for a system `7z.exe` first, then prompts before bootstrapping a local runtime into `<root>\bin\7zip`, and records the bootstrap version in `<root>\bin\7zip\VERSION`.
...
- The install flow also probes the downloaded temp file before final routing. Strong signatures such as MSI and archive containers can override a misleading manifest kind, and ZIP-shaped payloads are inspected for MSIX/AppX markers before the engine is finalized.
...
- extracts ZIP, Tar, or GZip payloads, or copies raw files
...
- [crates/engines/src/windows/msi.rs](../crates/engines/src/windows/msi.rs)
- [crates/engines/src/windows/msix/mod.rs](../crates/engines/src/windows/msix/mod.rs)
- [crates/engines/src/archive/install.rs](../crates/engines/src/archive/install.rs) for archive extraction once a 7z runtime is resolved
- [crates/app/src/operations/install/sevenz.rs](../crates/app/src/operations/install/sevenz.rs) for the 7z bootstrap-confirmation flow and version tracking
- [crates/windows/src/system/mod.rs](../crates/windows/src/system/mod.rs) for the `SearchPathW` system-binary check
- [crates/engines/src/portable/install.rs](../crates/engines/src/portable/install.rs)
...
- [crates/app/src/operations/repair/mod.rs](../crates/app/src/operations/repair/mod.rs)
