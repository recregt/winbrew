...
The crawler is the ingestion stage for the catalog pipeline. It fetches upstream package sources, normalizes them into a JSONL stream, and stages the Winget source database for the parser.
...
- Downloads the staged Winget source database to `--winget-out`.
...
- `--winget-out`: destination path for the staged Winget database.
...
- `--winget-out`: staged Winget database file.
...
2. a Winget database file on disk
...
go run ./cmd/crawler --config config.yaml --winget-out ..\staging\winget_source.db
```
