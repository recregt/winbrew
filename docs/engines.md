...
| `InstallerType::Zip` | `EngineKind::Zip` | Supported | WinBrew-owned filesystem engine | ZIP is the archive front door today. The shared archive dispatcher now handles ZIP, Tar, GZip, and 7z backends; 7z checks `SearchPathW` for a global `7z.exe` first and otherwise asks the install flow to bootstrap a local `bin/7zip` copy under the active WinBrew root after confirmation, and remove is still plain directory cleanup. The bootstrapped runtime now writes `bin/7zip/VERSION`, which Renovate can track through the 7-Zip bootstrap version constant. The install flow now probes the downloaded payload before final engine selection, so a mislabeled MSI or MSIX-shaped ZIP can still be routed correctly. |
...
- [crates/engines/src/archive/install.rs](../crates/engines/src/archive/install.rs)
