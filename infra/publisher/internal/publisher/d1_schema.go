...
const d1SchemaVersion = 1

func d1SchemaBootstrapStatements() []string {
	return []string{
		"CREATE TABLE IF NOT EXISTS schema_meta (id INTEGER PRIMARY KEY CHECK (id = 1), schema_version INTEGER NOT NULL);",
		fmt.Sprintf("INSERT OR REPLACE INTO schema_meta (id, schema_version) VALUES (1, %d);", d1SchemaVersion),
		"CREATE TABLE IF NOT EXISTS release_lineage (hash TEXT PRIMARY KEY, parent_hash TEXT, is_snapshot INTEGER NOT NULL DEFAULT 0, snapshot_url TEXT, metadata_url TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);",
		...
		"CREATE TABLE IF NOT EXISTS patch_artifacts (from_hash TEXT NOT NULL, to_hash TEXT NOT NULL, file_path TEXT NOT NULL, size_bytes INTEGER NOT NULL, checksum TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (from_hash, to_hash));",
