...
The parser is the transformation stage of the catalog pipeline. It consumes the crawler output plus the staged Winget JSONL file, writes the final SQLite catalog, and emits the catalog metadata sidecar.

## What it does

- Reads Scoop package envelopes from `stdin`.
- Loads the staged Winget JSONL file from `--winget-jsonl`.
...
- `--winget-jsonl`: staged Winget JSONL file produced by the crawler.
...
2. It reads Winget packages from the staged JSONL file.
...
cargo run --locked -- --winget-jsonl ..\staging\winget_source.jsonl --out ..\staging\catalog.db --metadata ..\staging\metadata.json
```
